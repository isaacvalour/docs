...
description: "Learn how to authenticate and secure your API calls with Valour API"
---

## Overview

Valour API uses JWT (JSON Web Tokens) for authentication. This guide covers all aspects of authentication including login, token management, 2FA, and security best practices.
...
```bash cURL
curl -X POST https://api.railsfromthecrypt.com/auth/refresh \
  -H "Content-Type: application/json" \
  -d '{
...
```javascript JavaScript
const response = await fetch('https://api.railsfromthecrypt.com/auth/refresh', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
response = requests.post(
    'https://api.railsfromthecrypt.com/auth/refresh',
    json={
        'refresh_token': 'your-refresh-token'
    }
)
...
// Usage
const client = new ValourAPIClient(
